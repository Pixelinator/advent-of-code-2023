...
public class Main {
	...
	public static void main ( String[] args ) {
		...
		for ( String arg :
				args ) {
			if ( arg.equals( "Game" ) && !input.toString().equals( "" ) ) {
				inputStrings.add( input.toString() );
				input = new StringBuilder();
			}
			input.append( arg );
		}
		inputStrings.add( input.toString() );
		ArrayList<CubeGame> cubeGames = new ArrayList<>();
		for ( String in :
				inputStrings ) {
			CubeGame cubeGame = new CubeGame( in );
			cubeGames.add( cubeGame );
		}
		int sum = 0;
		for ( CubeGame game :
				cubeGames ) {
			int power = game.numberOfRedCubes * game.numberOfGreenCubes * game.numberOfBlueCubes;
			sum += power;
		}
